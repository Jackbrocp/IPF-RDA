...
from IPF import Cutout as C


class Cutout_IPF(object):
    def __init__(self,length = 16, mean=[x / 255.0 for x in [125.3, 123.0, 113.9]], std=[x / 255.0 for x in [63.0, 62.1, 66.7]], early=False):
    
        self.length = length
        self.early = early
        
    def __call__(self, images, m, threshold):
        ...
        for (i,img) in enumerate(images_):
            ...
            while(True):
                ...
                if m[i][y1: y2, x1: x2].sum()  <= threshold[i] or threshold[i] == 0:
                    mask[y1: y2, x1: x2] = 0.
                    break
...
class HaS_IPF(object):
    def __init__(self, length, early=False):
        self.length = length
        self.early = early

    def __call__(self, ori_images, images, candidate):
        images_ = images.clone().detach()
        ...
        for i,img in enumerate(images_):
            ...
            idx = random.randint(0, len(candidate[i])-1)
            y1,y2,x1,x2 = candidate[i][idx]
            mask_zero[:,y1: y2, x1: x2] = ori_images[i,:,y1: y2, x1: x2].clone().detach().cpu()
...
class CutMix_IPF(object):
    def __init__(self,length = 16, mean=[x / 255.0 for x in [125.3, 123.0, 113.9]], std=[x / 255.0 for x in [63.0, 62.1, 66.7]], early=False):
    
        self.length = length
        self.early = early
        
    def __call__(self, images, importance, threshold):
        ...
        lam = torch.zeros(images.size()[0])
        for (i,img) in enumerate(images_):
            rand_img = images[rand_index[i]]
            while(True):
                y = np.random.randint(h)
                x = np.random.randint(w)
                ...
                if importance[i][y1: y2, x1: x2].sum()  <= threshold[i] or threshold[i] == 0:
                    img[:, y1: y2, x1: x2] = rand_img[:, y1: y2, x1: x2]
                    # lam[i] = (y2 - y1) * (x2 - x1) / 2 * (h * w)
                    if importance[rand_index[i]][y1: y2, x1: x2].sum() == 0:
                        lam[i] = 0
                    else:
                        lam[i] = importance[rand_index[i]][y1: y2, x1: x2].sum()/(importance[i].sum() - importance[i][y1: y2, x1: x2].sum() + importance[rand_index[i]][y1: y2, x1: x2].sum())
                    break
            images_[i] = img
        return images_.cuda(), lam.cuda(), rand_index.numpy()

class RandomErasing_IPF(object):
    def __init__(self,length = 16, early=False, mean=[0.4914, 0.4822, 0.4465]):
        self.length = length
        self.early = early
        self.mean = mean
        # self.p = 0.5
    ...
    def __call__(self, images, importance, threshold):
        ...
        for (i,img) in enumerate(images_):
            while(True):
                ...
                if importance[i][y1: y2, x1: x2].sum()  <= threshold[i] or threshold[i] == 0:
                    if img.size(0) ==3:
                        img[0, y1: y2, x1: x2] =  self.mean[0]
                        img[1, y1: y2, x1: x2] =  self.mean[1]
                        img[2, y1: y2, x1: x2] =  self.mean[2]
                    else:
                        img[0, y1: y2, x1: x2] =  self.mean[0]
                    break
            img = transforms.RandomHorizontalFlip()(img)
...
class GridMask_IPF(object):
    ...
    def __call__(self,ori_images, images, candidate):
        ...
        for i,img in enumerate(images_):
            mask_zero = torch.Tensor(np.zeros(img.shape, np.float32))
            idx = np.random.randint(0, len(candidate[i])-1)
            y1,y2,x1,x2 = candidate[i][idx]
            mask_zero[:,y1: y2, x1: x2] = ori_images[i,:,y1: y2, x1: x2].clone().detach().cpu()
            img[:,y1: y2, x1: x2] = mask_zero[:,y1: y2, x1: x2]
            img = transforms.RandomHorizontalFlip()(img)
...
class Randaugment_IPF(object):
    def __init__(self, length, N, M, early=False):
        self.length = length
        self.n = N
        self.m = M
        self.early = early
        self.augment_list = augment_list()
        self.cutout = C(1,length)
    ...
    def __call__(self,ori_images, images, candidate):
        ...
        for i,img in enumerate(images_):
            mask = np.ones((h, w), np.float32)
            mask_zero = torch.Tensor(np.zeros(img.shape, np.float32))
            idx = np.random.randint(len(candidate[i]))
            y1,y2,x1,x2 = candidate[i][idx]
            mask_zero[:,y1: y2, x1: x2] = ori_images[i,:,y1: y2, x1: x2].clone().detach().cpu()
            img[:,y1: y2, x1: x2] = mask_zero[:,y1: y2, x1: x2]
            img = self.cutout(img.cpu())
            images[i] = img
        return images.cuda()


class FastAugmentation_IPF(object):
    def __init__(self, length, early=False):
        self.length = length
        self.early = early
        self.cutout = C(1,length)
...
class Trivialaugment_IPF(object):
    ...
    def __call__(self,ori_images, images, candidate):
        ...
        for i,img in enumerate(images_):
            idx = random.randint(0, len(candidate[i])-1)
            y1,y2,x1,x2 = candidate[i][idx]
            img[:,y1: y2, x1: x2] = ori_images[i,:,y1: y2, x1: x2]
            img = self.cutout(img.cpu())
            img = transforms.RandomHorizontalFlip()(img)
            images[i] = img
        return images.cuda()
...
class Color_Auto(object):
    def __call__(self, x, magnitude):
        return ImageEnhance.Color(x).enhance(1 + magnitude * random.choice([-1, 1]))


class Posterize_Auto(object):
    def __call__(self, x, magnitude):
        magnitude = np.round(magnitude).astype(np.int)
        return ImageOps.posterize(x, magnitude)


class Solarize_Auto(object):
    def __call__(self, x, magnitude):
        return ImageOps.solarize(x, magnitude)


class Contrast_Auto(object):
    def __call__(self, x, magnitude):
        return ImageEnhance.Contrast(x).enhance(1 + magnitude * random.choice([-1, 1]))


class Sharpness_Auto(object):
    def __call__(self, x, magnitude):
        return ImageEnhance.Sharpness(x).enhance(1 + magnitude * random.choice([-1, 1]))


class Brightness_Auto(object):
    def __call__(self, x, magnitude):
        return ImageEnhance.Brightness(x).enhance(1 + magnitude * random.choice([-1, 1]))


class AutoContrast_Auto(object):
    def __call__(self, x, magnitude):
        return ImageOps.autocontrast(x)


class Equalize_Auto(object):
    def __call__(self, x, magnitude):
        return ImageOps.equalize(x)


class Invert_Auto(object):
    def __call__(self, x, magnitude):
...
class SubPolicy(object):
    def __init__(self, p1, operation1, magnitude_idx1, p2, operation2, magnitude_idx2, fillcolor=(128, 128, 128)):
        ...
        func = {
            "shearX": ShearX_Auto(fillcolor=fillcolor),
            "shearY": ShearY_Auto(fillcolor=fillcolor),
            "translateX": TranslateX_Auto(fillcolor=fillcolor),
            "translateY": TranslateY_Auto(fillcolor=fillcolor),
            "rotate": Rotate_Auto(),
            "color": Color_Auto(),
            "posterize": Posterize_Auto(),
            "solarize": Solarize_Auto(),
            "contrast": Contrast_Auto(),
            "sharpness": Sharpness_Auto(),
            "brightness": Brightness_Auto(),
            "autocontrast": AutoContrast_Auto(),
            "equalize": Equalize_Auto(),
            "invert": Invert_Auto()
        }
...
class CIFAR10Policy_magnitude(object):
    """ Randomly choose one of the best 25 Sub-policies on CIFAR10.
        Example:
        >>> policy = CIFAR10Policy()
        >>> transformed = policy(image)
        ...
        >>>     transforms.ToTensor()])
    """
    def __init__(self, M, fillcolor=(128, 128, 128)):
        self.policies = [
            SubPolicy_keepAA(0.1, "invert", 7, 0.2, "contrast", 6, M, fillcolor),
            SubPolicy_keepAA(0.7, "rotate", 2, 0.3, "translateX", 9, M, fillcolor),
            SubPolicy_keepAA(0.8, "sharpness", 1, 0.9, "sharpness", 3, M, fillcolor),
            SubPolicy_keepAA(0.5, "shearY", 8, 0.7, "translateY", 9, M, fillcolor),
            SubPolicy_keepAA(0.5, "autocontrast", 8, 0.9, "equalize", 2, M, fillcolor),

            SubPolicy_keepAA(0.2, "shearY", 7, 0.3, "posterize", 7, M, fillcolor),
            SubPolicy_keepAA(0.4, "color", 3, 0.6, "brightness", 7, M, fillcolor),
            SubPolicy_keepAA(0.3, "sharpness", 9, 0.7, "brightness", 9, M, fillcolor),
            SubPolicy_keepAA(0.6, "equalize", 5, 0.5, "equalize", 1, M, fillcolor),
            SubPolicy_keepAA(0.6, "contrast", 7, 0.6, "sharpness", 5, M, fillcolor),

            SubPolicy_keepAA(0.7, "color", 7, 0.5, "translateX", 8, M, fillcolor),
            SubPolicy_keepAA(0.3, "equalize", 7, 0.4, "autocontrast", 8, M, fillcolor),
            SubPolicy_keepAA(0.4, "translateY", 3, 0.2, "sharpness", 6, M, fillcolor),
            SubPolicy_keepAA(0.9, "brightness", 6, 0.2, "color", 8, M, fillcolor),
            SubPolicy_keepAA(0.5, "solarize", 2, 0.0, "invert", 3, M, fillcolor),

            SubPolicy_keepAA(0.2, "equalize", 0, 0.6, "autocontrast", 0, M, fillcolor),
            SubPolicy_keepAA(0.2, "equalize", 8, 0.6, "equalize", 4, M, fillcolor),
            SubPolicy_keepAA(0.9, "color", 9, 0.6, "equalize", 6, M, fillcolor),
            SubPolicy_keepAA(0.8, "autocontrast", 4, 0.2, "solarize", 8, M, fillcolor),
            SubPolicy_keepAA(0.1, "brightness", 3, 0.7, "color", 0, M, fillcolor),

            SubPolicy_keepAA(0.4, "solarize", 5, 0.9, "autocontrast", 3, M, fillcolor),
            SubPolicy_keepAA(0.9, "translateY", 9, 0.7, "translateY", 9, M, fillcolor),
            SubPolicy_keepAA(0.9, "autocontrast", 2, 0.8, "solarize", 3, M, fillcolor),
            SubPolicy_keepAA(0.8, "equalize", 8, 0.1, "invert", 3, M, fillcolor),
            SubPolicy_keepAA(0.7, "translateY", 9, 0.9, "autocontrast", 1, M, fillcolor)
        ]
...
class SubPolicy_keepAA(object):
    def __init__(self, p1, operation1, magnitude_idx1, p2, operation2, magnitude_idx2, M, fillcolor=(128, 128, 128)):
        ranges = {
            "shearX": [np.linspace(0, 0.3, 10)],
            "shearY": np.linspace(0, 0.3, 10),
            "translateX": np.linspace(0, 150 / 331, 10),
        ...
        self.p1 = p1
        self.operation1 = func[operation1]
        maxval1 = max(ranges[operation1][-1], ranges[operation1][0])
        minval1 = min(ranges[operation1][-1], ranges[operation1][0])
        self.magnitude1 = (float(M) / 30) * float(maxval1 - minval1) + minval1

        self.p2 = p2
        self.operation2 = func[operation2]
        maxval2 = max(ranges[operation2][-1], ranges[operation2][0])
        minval2 = min(ranges[operation2][-1], ranges[operation2][0])
        self.magnitude2 = (float(M) / 30) * float(maxval2 - minval2) + minval2
        # r op, minval, maxval in ops:
...
def augment_list_fast(for_autoaug=True):  # 16 oeprations and their ranges
    l = [
        (ShearX, -0.3, 0.3),  # 0
        (ShearY, -0.3, 0.3),  # 1
...
augment_dict = {fn.__name__: (fn, v1, v2) for fn, v1, v2 in augment_list_fast()}
